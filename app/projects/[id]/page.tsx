...
const project = {
  ...
  problem: "High-performance GPU computing in Africa was gated by extreme latency and fragmented resource allocation. Local AI startups were forced to use distant EU/US nodes, killing real-time inference capabilities.",
  solution: "We engineered a sovereign orchestration layer that abstracts bare-metal GPU clusters into a seamless, low-latency cloud. Using a custom Rust-based scheduler, we reduced node provisioning time by 85%.",
  metrics: [
    { label: "Provisioning", value: "12", unit: "s", desc: "Down from 3 minutes" },
    { label: "Latency", value: "18", unit: "ms", desc: "Regional round-trip" },
    { label: "Uptime", value: "99.99", unit: "%", desc: "Enterprise SLA" },
    { label: "Throughput", value: "100", unit: "Gbps", desc: "Inter-node fabric" },
  ],
  gallery: [
...
const SectionLabel = ({ text }: { text: string }) => (
  <div className="flex items-center gap-3 mb-8">
    ...
    <span className="text-[10px] font-mono uppercase tracking-[0.4em] text-slate-400 font-bold">{text}</span>
  </div>
);

// ─── INTERACTIVE GALLERY ITEM ──────────────────────────────────────────────

const InteractiveGalleryItem = ({ img, index }: { img: GalleryImage; index: number }) => {
        ...
        <div className="relative w-full h-full overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[9px] font-mono text-blue-400 font-bold uppercase tracking-widest">{img.sub}</span>
            </div>
            <h4 className="text-xl md:text-2xl font-medium text-white tracking-tight">{img.label}</h4>
          </motion.div>
...
export default function CaseStudyPage() {
  ...
  return (
    <div ref={containerRef} className="bg-white selection:bg-blue-600 selection:text-white antialiased">
        <Navbar />
      
      {/* ── 1. HERO ── */}
      <section className="relative h-screen w-full flex flex-col justify-center overflow-hidden border-b border-slate-100">
        ...
        <div className="container relative z-10 px-6 mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <Link href="/projects" className="group inline-flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-end">
            <div className="lg:col-span-9">
              <motion.h1 
                ...
                className="text-7xl md:text-9xl lg:text-[11rem] font-medium tracking-tighter leading-[0.8] text-slate-900"
              >
                {project.title}
              </motion.h1>
            </div>
            <div className="lg:col-span-3 text-right">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="flex flex-col gap-1">
                <span className="text-[10px] font-mono uppercase tracking-[0.3em] text-slate-400 font-bold">Client: {project.client}</span>
                <span className="text-[10px] font-mono uppercase tracking-[0.3em] text-slate-400 font-bold">Project Year: {project.year}</span>
              </motion.div>
            </div>
          </div>
        </div>
        ...
        <motion.div 
          ...
          className="absolute bottom-[-15%] left-1/2 -translate-x-1/2 w-[90vw] h-[55vh] z-0 shadow-2xl overflow-hidden"
        >
          <img 
        ...
        <div className="container px-6 mx-auto relative z-10">
          <div className="max-w-4xl">
            ...
            <motion.h2 
              ...
              viewport={{ once: true }}
              className="text-4xl md:text-6xl font-medium tracking-tight text-slate-900 leading-[1.1] mb-12"
            >
              {project.problem}
            </motion.h2>
          </div>
        ...
        <div className="container px-6 mx-auto">
          <div className="grid lg:grid-cols-12 gap-20 items-center">
            <div className="lg:col-span-5">
              ...
              <h3 className="text-4xl font-medium mb-8 tracking-tight text-slate-900">Architecture for Global Scale.</h3>
              ...
              <div className="space-y-4">
                {project.tags.map((tag, i) => (
                  ...
                  >
                    <Terminal size={16} className="text-blue-600 group-hover:scale-110 transition-transform" />
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-700">{tag}</span>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 md:mb-24 gap-10">
            <div className="max-w-2xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-2 h-2 rounded-full bg-blue-600" />
                <span className="text-[10px] font-mono uppercase tracking-[0.4em] text-slate-400 font-bold">Visual Documentation</span>
              </div>
              <h2 className="text-5xl md:text-7xl font-medium tracking-tight text-slate-900">System Interface.</h2>
            </div>
            <div className="flex items-center gap-4 text-slate-400 self-end">
              <MousePointer2 size={16} className="animate-bounce" />
              <span className="text-[10px] font-mono uppercase tracking-widest">Hover to explore depth</span>
            </div>
          </div>
        ...
        <div className="container px-6 mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-px bg-slate-200 border border-slate-200">
            {project.metrics.map((metric, i) => (
              <div key={i} className="bg-white p-12 transition-colors hover:bg-slate-50 group">
                <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest block mb-6">{metric.label}</span>
                <div className="flex items-baseline gap-1 mb-2">
                  <span className="text-5xl font-bold tracking-tighter text-slate-900 group-hover:text-blue-600 transition-colors">{metric.value}</span>
                  <span className="text-xl font-medium text-blue-600">{metric.unit}</span>
                </div>
                <p className="text-xs text-slate-400 font-mono italic">{metric.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container px-6 mx-auto relative z-10 text-center">
          ...
          >
            <h2 className="text-6xl md:text-8xl text-white font-medium tracking-tighter leading-[0.9] mb-12">
              Scale your system <br />with <span className="text-blue-500 italic">authority.</span>
            </h2>
